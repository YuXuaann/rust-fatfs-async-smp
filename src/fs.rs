...
impl FsInfoSector {
    ...
    async fn deserialize<R: Read + Send>(rdr: &mut R) -> Result<Self, Error<R::Error>> {
    ...
    async fn serialize<W: Write + Send>(&self, wrt: &mut W) -> Result<(), Error<W::Error>> {
...
unsafe impl<IO: ReadWriteSeek + Send, TP, OCC> Send for FileSystem<IO, TP, OCC> {}
unsafe impl<IO: ReadWriteSeek + Send, TP, OCC> Sync for FileSystem<IO, TP, OCC> {}
...
impl<IO: Read + Write + Seek + Send, TP, OCC> FileSystem<IO, TP, OCC> {
    ...
    pub(crate) async fn truncate_cluster_chain(self: &Arc<Self>, cluster: u32) -> Result<(), Error<IO::Error>> {
    ...
    pub(crate) async fn free_cluster_chain(self: &Arc<Self>, cluster: u32) -> Result<(), Error<IO::Error>> {
        let mut iter = self.cluster_iter(cluster);
    ...
    pub async fn read_status_flags(self: &Arc<Self>) -> Result<FsStatusFlags, Error<IO::Error>> {
    ...
    ///
    /// `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub async fn stats(self: &Arc<Self>) -> Result<FileSystemStats, Error<IO::Error>> {
    ...
    async fn recalc_free_clusters(self: &Arc<Self>) -> Result<u32, Error<IO::Error>> {
        let mut fat = self.fat_slice();
...
impl<IO: ReadWriteSeek + Send, TP, OCC> Read for FsIoAdapter<IO, TP, OCC> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        self.fs.disk.lock().read(buf).await
    }
}
...
impl<IO: ReadWriteSeek + Send, TP, OCC> Write for FsIoAdapter<IO, TP, OCC> {
    async fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        let size = self.fs.disk.lock().write(buf).await?;
        if size > 0 {
...
fn fat_slice<S: ReadWriteSeek + Send, B: BorrowMut<S> + Send>(
    io: B,
    bpb: &BiosParameterBlock,
...
#[async_trait]
impl<B: BorrowMut<S> + Send, S: Read + Seek + Send> Read for DiskSlice<B, S> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
...
#[async_trait]
impl<B: BorrowMut<S> + Send, S: Write + Seek + Send> Write for DiskSlice<B, S> {
    async fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        let offset = self.begin + self.offset;
...
pub(crate) async fn write_zeros<IO: ReadWriteSeek + Send>(disk: &mut IO, mut len: u64) -> Result<(), IO::Error> {
...
async fn write_zeros_until_end_of_sector<IO: ReadWriteSeek + Send>(
    disk: &mut IO,
...
pub async fn format_volume<S: ReadWriteSeek + Send>(
    storage: &mut S,
